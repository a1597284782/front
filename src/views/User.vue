<template>
  <div>
    <div class="fly-home fly-panel" style="background-image: url()">
      <img :src="userInfo.pic ? userInfo.pic : ''" alt="贤心" />
      <h1>
        {{ userInfo.name }}
      ...
      <div class="layui-row layui-col-space15">
        <div class="layui-col-md6 fly-home-jie">
          <div class="fly-panel">
            ...
            <ul class="jie-row">
              <li v-for="item in postList" :key="'postList' + item._id">
                <span
                  class="layui-badge"
        ...
        <div class="layui-col-md6 fly-home-da">
          <div class="fly-panel">
            ...
            <ul class="home-jieda">
              <li v-for="item in commentList" :key="'commentList' + item._id">
                <p>
                  <span>{{ item.created | moment }}</span>
                  在
                  <router-link
                    :to="{
                      name: item.tid ? 'detail' : '404',
                      params: { tid: item.tid ? item.tid._id : '' },
                    }"
                    >{{ item.tid ? item.tid.title : '(用户已经删除该帖)' }}
                  </router-link>
                  中回答：
                </p>
...
<script>
import { getInfo, getCommentList, getPostListPublic } from '@/api/user'
